    ...
    */
        ...
        */
        private static int sqlite3IsNumber(string z, ref int realnum, int enc)
        {
            ...
            zIndex += incr; //z += incr;
            realnum = 0;
            while (zIndex < z.Length && sqlite3Isdigit(z[zIndex])) zIndex += incr;
#if !SQLITE_OMIT_FLOATING_POINT
...
return sqlite3Atoi64(z, pResult);
        ...
        */
        private static bool sqlite3Atoi64(string zNum, ref long pNum)
        {
            zNum = zNum.Trim() + " ";
            int i;
            for (i = 1; i < zNum.Length; i++)
                if (!sqlite3Isdigit(zNum[i]))
                    break;
            var c = long.TryParse(zNum.Substring(0, i), out pNum);
        ...
        */
        private static int getVarint(byte[] p, ref uint v)
        {
            v = p[0];
            if (v <= 0x7F) return 1;
            ulong u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref uint v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
            ulong u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref long v)
        {
        ...
        private static int getVarint(byte[] p, int offset, ref ulong v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, ref uint v)
        {
        ...
        private static int getVarint32(string s, uint offset, ref int v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = s[(int) offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(string s, uint offset, ref uint v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = s[(int) offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, uint offset, ref uint v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = p[offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, int offset, ref uint v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
        ...
        */
        private static byte sqlite3GetVarint32(byte[] p, ref int v)
        {
            uint u32_v = 0;
        ...
        private static byte sqlite3GetVarint32(byte[] p, int offset, ref int v)
        {
            uint u32_v = 0;
        ...
        private static byte sqlite3GetVarint32(byte[] p, ref uint v)
        {
            return sqlite3GetVarint32(p, 0, ref v);
        }
