...
** memory as necessary.
*/
        public static int sqlite3_get_table_cb(object pArg, long nCol, object Oargv, object Ocolv)
        {
            var argv = (string[]) Oargv;
            var colv = (string[]) Ocolv;
            var p = (TabResult) pArg;
            int need;
            int i;
            string z;
...
*/
            if (argv != null)
            {
                for (i = 0; i < nCol; i++)
                {
                    ...
                    {
                        var n = sqlite3Strlen30(argv[i]) + 1;
//z = sqlite3_malloc( n );
...
*/
        ...
        )
        {
            ...
            res.azResult[0] = null;
            rc = sqlite3_exec(db, zSql, sqlite3_get_table_cb, res, ref pzErrMsg);
...
//sqlite3_free_table(ref res.azResult[1] );
                if (res.zErrMsg != "")
                    if (pzErrMsg != null) //sqlite3_free(ref pzErrMsg);
...
//sqlite3_free(ref res.zErrMsg);
            if (rc != SQLITE_OK) //sqlite3_free_table(ref res.azResult[1]);
                return rc;
            if (res.nAlloc > res.nData)
            {
                string[] azNew;
                Array.Resize(ref res.azResult,
                    res.nData - 1); //sqlite3_realloc( res.azResult, sizeof(char*)*(res.nData+1) );
//if( azNew==null ){
//  //sqlite3_free_table(ref res.azResult[1]);
//  db.errCode = SQLITE_NOMEM;
//  return SQLITE_NOMEM;
...
*/
        private static void sqlite3_free_table(
