...
#endif
                    || (opcode == OP_Halt || opcode == OP_HaltIfNull) && pOp.p1 == SQLITE_CONSTRAINT &&
                    pOp.p2 == OE_Abort
                )
                {
                    hasAbort = 1;
...
if( n>nMaxArgs ) nMaxArgs = n;
        ...
        */
        private static void sqlite3VdbeProgramDelete(sqlite3 db, SubProgram p, int freeop)
        {
            if (p != null)
            {
...
#if SQLITE_DEBUG
            for (n = 1; n < p.nMem; n++) Debug.Assert(p.aMem[n].db == db);
#endif
...
fclose(out);
        ...
        */
        private static uint sqlite3VdbeSerialTypeLen(uint serial_type)
        {
            ...
            uint[] aSize = { 0, 1, 2, 3, 4, 6, 8, 8, 0, 0, 0, 0 };
...
swapMixedEndianFloat(t2);
            ...
            Debug.Assert( sizeof( double ) == sizeof( u64 ) && memcmp( BitConverter.GetBytes( r1 ), BitConverter.GetBytes( t2 ), sizeof( double ) ) == 0 );//Debug.Assert( sizeof(r1)==sizeof(t2) && memcmp(&r1, t2, sizeof(r1))==0 );
                        ...
                        x = (ulong)((buf[offset + 0] << 24) | (buf[offset + 1] << 16) |
                                     (buf[offset + 2] << 8) | buf[offset + 3]);
        ...
        */
        ...
        )
        {
            ...
            */
            ...
            idx = (uint)getVarint32(aKey, 0, ref szHdr); // GetVarint( aKey, szHdr );
            d = szHdr;
            u = 0;
            while (idx < szHdr && u < p.nField && d <= nKey)
            {
                ...
                idx += (uint)getVarint32(aKey, idx,
                    ref serial_type); // GetVarint( aKey + idx, serial_type );
        ...
        )
        {
            ...
            */
            ...
            idx1 = (uint)((szHdr1 = pKey1[offset]) <= 0x7f
                ? 1
                : getVarint32(pKey1, offset, ref szHdr1)); // GetVarint( aKey1, szHdr1 );
            d1 = (int)szHdr1;
            ...
            while (idx1 < szHdr1 && i < pPKey2.nField)
            {
                ...
                idx1 += (uint)((serial_type1 = pKey1[offset + idx1]) <= 0x7f
                        ? 1
                        : getVarint32(pKey1, (uint)(offset + idx1), ref serial_type1)
