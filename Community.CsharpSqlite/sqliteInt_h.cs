...
public   sqlite3 db;                    /* "Owner" connection. See comment above */
        ...
        */
        public class sqlite3
        {
            ...
            public byte dfltJournalMode; /* Default journal mode for attached dbs */
...
sqlite3 *pNextBlocked;        /* Next in list of all blocked connections */
        ...
        */
        ...
        private const int SQLITE_VdbeTrace = 0x00000100; /* True to trace VDBE execution */
        private const int SQLITE_InternChanges = 0x00000200; /* Uncommitted Hash table changes */
        private const int SQLITE_FullColNames = 0x00000400; /* Show full column names on SELECT */
        private const int SQLITE_ShortColNames = 0x00000800; /* Show short columns names */

        private const int SQLITE_CountRows = 0x00001000; /* Count rows changed by INSERT, */

        /*   DELETE, or UPDATE and return */
        /*   the count using a callback. */
        private const int SQLITE_NullCallback = 0x00002000; /* Invoke the callback once if the */

        /*   result set is empty */
        private const int SQLITE_SqlTrace = 0x00004000; /* Debug print SQL as it executes */
        private const int SQLITE_VdbeListing = 0x00008000; /* Debug listings of VDBE programs */
        private const int SQLITE_WriteSchema = 0x00010000; /* OK to update SQLITE_MASTER */

        private const int SQLITE_NoReadlock = 0x00020000; /* Readlocks are omitted when 
                              ** accessing read-only databases */

        private const int SQLITE_IgnoreChecks = 0x00040000; /* Do not enforce check constraints */
        private const int SQLITE_ReadUncommitted = 0x0080000; /* For shared-cache mode */
        private const int SQLITE_LegacyFileFmt = 0x00100000; /* Create new databases in format 1 */
        private const int SQLITE_FullFSync = 0x00200000; /* Use full fsync on the backend */
        private const int SQLITE_LoadExtension = 0x00400000; /* Enable load_extension */
        private const int SQLITE_RecoveryMode = 0x00800000; /* Ignore schema errors */
        private const int SQLITE_ReverseOrder = 0x01000000; /* Reverse unordered SELECTs */
        private const int SQLITE_RecTriggers = 0x02000000; /* Enable recursive triggers */
        private const int SQLITE_ForeignKeys = 0x04000000; /* Enforce foreign key constraints  */
        ...
        */
        ...
        private const int SQLITE_QueryFlattener = 0x01; /* Disable query flattening */
        private const int SQLITE_ColumnCache = 0x02; /* Disable the column cache */
        private const int SQLITE_IndexSort = 0x04; /* Disable indexes for sorting */
        private const int SQLITE_IndexSearch = 0x08; /* Disable indexes for searching */
        private const int SQLITE_IndexCover = 0x10; /* Disable index covering table */
        private const int SQLITE_OptMask = 0x1f; /* Mask of all disablable opts */
    ...
    */
        ...
        */
        ...
        public class Parse
        {
            ...
            public uint cookieMask; /* Bitmask of schema verified databases */
