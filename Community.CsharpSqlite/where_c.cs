...
namespace Community.CsharpSqlite
{
    ...
    */
        ...
        */
        ...
        {
            public ushort eOperator; /* A WO_xx value describing <op> */
            public int iParent; /* Disable pWC.a[iParent] when this term disabled */
            public int leftCursor; /* Cursor number of X in "X <op> <expr>" */
            public byte nChild; /* Number of children that must disable us */
            public Expr pExpr; /* Pointer to the subexpression that is this term */
            public ulong prereqAll; /* Bitmask of tables referenced by pExpr */
            public ulong prereqRight; /* Bitmask of tables used by pExpr.pRight */
            public WhereClause pWC; /* The clause this term is part of */
            public _u u = new _u();
            public byte wtFlags; /* TERM_xxx bit flags.  See below */
        ...
        */
        public class WhereClause
        {
            ...
            public ulong vmask; /* Bitmask identifying virtual table cursors */
...
public WhereTerm[] aStatic = new WhereTerm[1];    /* Initial static space for a[] */
        ...
        */
        ...
        {
            public ulong indexable; /* Bitmask of all indexable tables in the clause */
        ...
        */
        public class WhereCost
        {
            ...
            public ulong used; /* Bitmask of cursors used by this plan */
        ...
        */
        private static void whereClauseClear(WhereClause pWC)
        {
            ...
            if (pWC.a != pWC.aStatic) sqlite3DbFree(db, ref pWC.a);
        }
        ...
        */
        ...
        {
            int i;
            Debug.Assert(pMaskSet.n <= sizeof(ulong) * 8);
        ...
        */
        ...
        {
            var expRight = (ushort)(pExpr.pRight.flags & EP_ExpCollate);
            var expLeft = (ushort)(pExpr.pLeft.flags & EP_ExpCollate);
            Debug.Assert(allowedOp(pExpr.op) && pExpr.op != TK_IN);
            pExpr.pRight.pColl = sqlite3ExprCollSeq(pParse, pExpr.pRight);
            pExpr.pLeft.pColl = sqlite3ExprCollSeq(pParse, pExpr.pLeft);
            SWAP(ref pExpr.pRight.pColl, ref pExpr.pLeft.pColl);
            pExpr.pRight.flags = (ushort)((pExpr.pRight.flags & ~EP_ExpCollate) | expLeft);
            pExpr.pLeft.flags = (ushort)((pExpr.pLeft.flags & ~EP_ExpCollate) | expRight);
            ...
            if (pExpr.op >= TK_GT)
            {
                ...
                pExpr.op = (byte)(((pExpr.op - TK_GT) ^ 2) + TK_GT);
            }
        }
        ...
        */
        private static WhereTerm findTerm(
            ...
            ulong notReady, /* RHS must not overlap with this mask */
            uint op, /* Mask of WO_xx values describing operator */
...
if( pnoCase ) return 0;
            ...
            op = pRight.op;
            if (op == TK_REGISTER) op = pRight.op2;
            if (op == TK_VARIABLE)
            {
                var pReprepare = pParse.pReprepare;
                pVal = sqlite3VdbeGetValue(pReprepare, pRight.iColumn, (byte)SQLITE_AFF_NONE);
                if (pVal != null && sqlite3_value_type(pVal) == SQLITE_TEXT) z = sqlite3_value_text(pVal);
...
return 1;
    ...
    */
        ...
        {
            pDerived.flags = (ushort)(pDerived.flags | (pBase.flags & EP_FromJoin));
    ...
    */
        ...
        )
        {
            ...
            ulong chngToIN; /* Tables that might satisfy case 1 */
            ulong indexable; /* Tables that are indexable, satisfying case 2 */
            ...
            */
            ...
            pTerm.eOperator = (ushort)(indexable == 0 ? 0 : WO_OR);
                ...
                */
                for (j = 0; j < 2 && 0 == okToChngToIN; j++)
                {
                    ...
                    for (i = pOrWc.nTerm - 1; i >= 0; i--) //, pOrTerm++)
                    {
                        ...
                        pOrTerm.wtFlags = (byte)(pOrTerm.wtFlags & ~TERM_OR_OK);
    ...
    */
        ...
        )
        {
            ...
            ulong prereqLeft; /* Prerequesites of the pExpr.pLeft */
            ulong prereqAll; /* Prerequesites of pExpr */
            ulong extraRight = 0; /* Extra dependencies on LEFT JOIN */
            Expr pStr1 = null; /* RHS of LIKE/GLOB operator */
            var isComplete = false; /* RHS of LIKE/GLOB ends with wildcard */
            var noCase = false; /* LIKE/GLOB distinguishes case */
        ...
        )
        {
            ...
            */
            ...
            if (pIdx.onError != OE_None && i == pIdx.nColumn
                                        && !referencesOtherTables(pOrderBy,
                                            pMaskSet, j, _base))
    ...
    */
        ...
        */
        private static void bestOrClauseIndex(
            ...
            ulong notReady, /* Mask of cursors that are not available */
            ExprList pOrderBy, /* The ORDER BY clause */
        ...
        )
        {
            ...
            var iCur = pSrc
                .iCursor; /* The cursor of the table to be accessed */
            var maskSrc = getMask(pWC.pMaskSet,
                iCur); /* Bitmask for pSrc */
                ...
                _pt++) //<pWCEnd; pTerm++)
            {
                ...
                )
                {
                    ...
                    for (var _pOrWC = 0;
                        _pOrWC < pOrWC.nTerm;
                        _pOrWC
                            ++) //pOrTerm = pOrWC.a ; pOrTerm < pOrWCEnd ; pOrTerm++ )
                    {
            ...
            _pOrWC, pOrWC.nTerm - _pOrWC//( pOrTerm - pOrWC.a ), ( pTerm - pWC.a )
                        ...
                        if (pOrTerm.eOperator == WO_AND)
                        {
                            ...
                            bestIndex(pParse, pAndWC, pSrc,
                                notReady, null, ref sTermCost);
                        }
  ...
  return sqlite3ValueFromExpr(pParse.db, pExpr, SQLITE_UTF8, aff, pp);
    ...
    */
        private static int whereRangeScanEst(
            ...
            WhereTerm
                pLower, /* Lower bound on the range. ex: "x>123" Might be NULL */
            WhereTerm
                pUpper, /* Upper bound on the range. ex: "x<455" Might be NULL */
...
range_est_fallback:
        ...
        */
        ...
        )
        {
            ...
            uint
                eqTermMask; /* Current mask of valid equality operators */
            uint
            ...
            */
            ...
            else
            {
                ...
                ** represent the primary key */
                ...
                sPk =
                    new Index(); // memset( &sPk, 0, sizeof( Index ) );
                ...
                if (pSrc.notIndexed == 0) sPk.pNext = pFirst;
                ...
                */
                if (pFirst != null)
                {
                    Debug.Assert(
                        pFirst.aiRowEst !=
                        null); /* Allocated together with pFirst */
                ...
                pProbe = sPk;
                wsFlagMask = ~(
                    WHERE_COLUMN_IN | WHERE_COLUMN_EQ |
                    WHERE_COLUMN_NULL | WHERE_COLUMN_RANGE
                );
            ...
            */
            ...
            {
                var aiRowEst = pProbe.aiRowEst;
                double cost; /* Cost of using pProbe */
                double
                    nRow; /* Estimated number of rows in result set */
                ...
                */
                ...
                {
                    WhereTerm
                        pTerm; /* A single term of the WHERE clause */
                    var j = pProbe.aiColumn[nEq];
                    pTerm = findTerm(pWC, iCur, j, notReady,
                        eqTermMask, pIdx);
                    if (pTerm == null) break;
                ...
                {
                    var j = pProbe.aiColumn[nEq];
                    if (findTerm(pWC, iCur, j, notReady,
                            WO_LT | WO_LE | WO_GT | WO_GE, pIdx) !=
                        null)
                    {
                        var pTop = findTerm(pWC, iCur, j, notReady,
                            WO_LT | WO_LE, pIdx);
                        var pBtm = findTerm(pWC, iCur, j, notReady,
                            WO_GT | WO_GE, pIdx);
                        whereRangeScanEst(pParse, pProbe, nEq, pBtm,
                            pTop, ref nBound);
                        if (pTop != null)
                        {
                ...
                else if (pProbe.onError != OE_None)
                {
                    ...
                    if ((wsFlags & (WHERE_COLUMN_IN |
                                    WHERE_COLUMN_NULL)) ==
                        0) wsFlags |= WHERE_UNIQUE;
                }
                ...
                ** will scan rows in a different order, set the bSort variable.  */
                if (pOrderBy != null)
                {
                    if ((wsFlags & (WHERE_COLUMN_IN |
                                    WHERE_COLUMN_NULL)) == 0
                        && isSortingIndex(pParse, pWC.pMaskSet,
                            pProbe, iCur, pOrderBy, nEq, ref rev)
                    )
                    {
                ...
                ** wsFlags. Otherwise, set the bLookup variable to true.  */
                if (pIdx != null && wsFlags != 0)
                {
                    ...
                    {
                        var x = pIdx.aiColumn[j];
                        if (x < BMS - 1) m &= ~((ulong)1 << x);
                    }
                ...
                */
                nRow = nRow * nBound / 100;
                cost = cost * nBound / 100;

                /* Add in the estimated cost of sorting the result
                */
                if (bSort != 0) cost += cost * estLog(cost);
                ...
                */
                ...
                WHERETRACE(
                    ...
                    pSrc.pTab.zName,
                    pIdx != null ? pIdx.zName : "ipk",
                    nEq, nInMul, nBound, bSort, bLookup, wsFlags,
                    nRow, cost
                );
            ...
            ** SQLite outputs rows in in the absence of an ORDER BY clause.  */
            if (null == pOrderBy &&
                (pParse.db.flags & SQLITE_ReverseOrder) != 0)
                pCost.plan.wsFlags |= WHERE_REVERSE;

            Debug.Assert(
                pOrderBy != null ||
                (pCost.plan.wsFlags & WHERE_ORDERBY) == 0);
            Debug.Assert(
                pCost.plan.u.pIdx == null ||
                (pCost.plan.wsFlags & WHERE_ROWID_EQ) == 0);
            ...
            WHERETRACE("best index is: %s\n",
                pCost.plan.u.pIdx != null
                    ? pCost.plan.u.pIdx.zName
                    : "ipk"
            );

            bestOrClauseIndex(pParse, pWC, pSrc, notReady, pOrderBy,
                pCost);
...
sqlite3DbFree(pParse.db, p);
...
#endif
            {
                bestBtreeIndex(pParse, pWC, pSrc, notReady,
                    pOrderBy, ref pCost);
            }
        }
        ...
        */
        private static void disableTerm(WhereLevel pLevel,
            WhereTerm pTerm)
        {
            if (pTerm != null
        ...
        */
        private static void codeApplyAffinity(Parse pParse,
            int _base, int n, string zAff)
        {
            var v = pParse.pVdbe;
            //if (zAff == 0)
            //{
            ...
            */
            ...
            while (n > 1 && zAff[n - 1] == SQLITE_AFF_NONE) n--;
        ...
        */
        private static int codeEqualityTerm(
            ...
            WhereLevel
                pLevel, /* When level of the FROM clause we are working on */
        ...
        )
        {
            ...
            if (pX.op == TK_EQ)
            {
                iReg = sqlite3ExprCodeTarget(pParse, pX.pRight,
                    iTarget);
            }
            ...
            else
            {
                ...
                Debug.Assert(
                    (pLevel.plan.wsFlags & WHERE_IN_ABLE) != 0);
                if (pLevel.u._in.nIn == 0)
                    pLevel.addrNxt = sqlite3VdbeMakeLabel(v);
                pLevel.u._in.nIn++;
                if (pLevel.u._in.aInLoop == null)
                    pLevel.u._in.aInLoop =
                        new InLoop[pLevel.u._in.nIn];
                else
                    Array.Resize(ref pLevel.u._in.aInLoop,
                        pLevel.u._in.nIn);
                ...
                {
                    pLevel.u._in.aInLoop[pLevel.u._in.nIn - 1] =
                        new InLoop();
                    pIn = pLevel.u._in.aInLoop[
                        pLevel.u._in.nIn - 1]; //pIn++
                    pIn.iCur = iTab;
                    if (eType == IN_INDEX_ROWID)
                        pIn.addrInTop = sqlite3VdbeAddOp2(v,
                            OP_Rowid, iTab, iReg);
                    else
                        pIn.addrInTop = sqlite3VdbeAddOp3(v,
                            OP_Column, iTab, 0, iReg);
        ...
        */
        private static int codeAllEqualityTerms(
            ...
            WhereClause pWC, /* The WHERE clause */
            ulong notReady, /* Which parts of FROM have not yet been coded */
            int nExtraReg, /* Number of extra registers to allocate */
            ref StringBuilder
                pzAff /* OUT: Set to point to affinity string */
        )
        {
            var nEq = (int)pLevel.plan
                .nEq; /* The number of == or IN constraints to code */
            ...
            */
            ...
            for (j = 0; j < nEq; j++)
            {
                ...
                pTerm = findTerm(pWC, iCur, k, notReady,
                    pLevel.plan.wsFlags, pIdx);
                if (NEVER(pTerm == null)) break;
                Debug.Assert((pTerm.wtFlags & TERM_CODED) == 0);
                r1 = codeEqualityTerm(pParse, pTerm, pLevel,
                    regBase + j);
                if (r1 != regBase + j)
                {
                    if (nReg == 1)
                    ...
                    {
                        sqlite3VdbeAddOp2(v, OP_SCopy, r1,
                            regBase + j);
                    }
                }
                ...
                {
                    var pRight = pTerm.pExpr.pRight;
                    sqlite3ExprCodeIsNullJump(v, pRight,
                        regBase + j, pLevel.addrBrk);
                    if (zAff.Length > 0)
                    {
                        if (sqlite3CompareAffinity(pRight,
                            zAff[j]) == SQLITE_AFF_NONE)
                            zAff[j] = SQLITE_AFF_NONE;
                        if (sqlite3ExprNeedsNoAffinityChange(pRight,
                            zAff[j]) != 0)
                            zAff[j] = SQLITE_AFF_NONE;
                    }
                }
        ...
        */
        private static ulong codeOneLoopStart(
            ...
            ushort
                wctrlFlags, /* One of the WHERE_* flags defined in sqliteInt.h */
            ulong notReady /* Which tables are currently available */
        )
        {
            int j, k; /* Loop counters */
            int iCur; /* The VDBE cursor for the table */
            int
                addrNxt; /* Where to jump to continue with the next IN case */
            int omitTable; /* True if we use the index only */
            int bRev; /* True if we need to scan in reverse order */
            WhereLevel pLevel; /* The where level to be coded */
            WhereClause
                pWC; /* Decomposition of the entire WHERE clause */
            WhereTerm pTerm; /* A WHERE clause term */
            Parse pParse; /* Parsing context */
            Vdbe v; /* The prepared stmt under constructions */
            SrcList_item
                pTabItem; /* FROM clause term being coded */
            int addrBrk; /* Jump here to break out of the loop */
            int
                addrCont; /* Jump here to continue with next cycle */
            var iRowidReg =
...
sqlite3ExprCachePop(pParse, 1);
            ...
            if ((pLevel.plan.wsFlags & WHERE_ROWID_EQ) != 0)
            {
                ...
                */
                ...
                pTerm = findTerm(pWC, iCur, -1, notReady,
                    WO_EQ | WO_IN, null);
                Debug.Assert(pTerm != null);
                Debug.Assert(pTerm.pExpr != null);
                Debug.Assert(pTerm.leftCursor == iCur);
                Debug.Assert(omitTable == 0);
                iRowidReg = codeEqualityTerm(pParse, pTerm, pLevel,
                    iReleaseReg);
                addrNxt = pLevel.addrNxt;
                sqlite3VdbeAddOp2(v, OP_MustBeInt, iRowidReg,
                    addrNxt);
                sqlite3VdbeAddOp3(v, OP_NotExists, iCur, addrNxt,
                    iRowidReg);
            ...
            else if ((pLevel.plan.wsFlags & WHERE_ROWID_RANGE) != 0)
            {
                ...
                */
                ...
                Debug.Assert(omitTable == 0);
                pStart = findTerm(pWC, iCur, -1, notReady,
                    WO_GT | WO_GE, null);
                pEnd = findTerm(pWC, iCur, -1, notReady,
                    WO_LT | WO_LE, null);
                if (bRev != 0)
                {
                    pTerm = pStart;
                ...
                {
                    Expr
                        pX; /* The expression that defines the start bound */
                    int r1,
                        rTemp =
                            0; /* Registers for holding the start boundary */
...
/* TK_GE */ OP_SeekGe
                    ...
                    r1 = sqlite3ExprCodeTemp(pParse, pX.pRight,
                        ref rTemp);
                    sqlite3VdbeAddOp3(v, aMoveOp[pX.op - TK_GT],
                        iCur, addrBrk, r1);
#if SQLITE_DEBUG
                    VdbeComment(v, "pk");
#endif
                ...
                pLevel.op = (byte)(bRev != 0 ? OP_Prev : OP_Next);
                pLevel.p1 = iCur;
                pLevel.p2 = start;
                pLevel.p5 = (byte)(pStart == null && pEnd == null
                    ? 1
                    : 0);
                if (testOp != OP_Noop)
                {
                    iRowidReg = iReleaseReg =
                        sqlite3GetTempReg(pParse);
                    sqlite3VdbeAddOp2(v, OP_Rowid, iCur, iRowidReg);
                    sqlite3ExprCacheStore(pParse, iCur, -1,
                        iRowidReg);
                    sqlite3VdbeAddOp3(v, testOp, memEndValue,
                        addrBrk, iRowidReg);
                    sqlite3VdbeChangeP5(v,
                        SQLITE_AFF_NUMERIC | SQLITE_JUMPIFNULL);
                }
            }
            else if ((pLevel.plan.wsFlags &
                      (WHERE_COLUMN_RANGE | WHERE_COLUMN_EQ)) != 0)
            {
                ...
                */
                ...
                int
                    regBase; /* Base register holding constraint values */
                int r1; /* Temp register */
                WhereTerm pRangeStart =
                    null; /* Inequality constraint at range start */
                WhereTerm pRangeEnd =
                    null; /* Inequality constraint at range end */
                int
                    startEq; /* True if range start uses ==, >= or <= */
                int endEq; /* True if range end uses ==, >= or <= */
                int
                    start_constraints; /* Start of range is constrained */
                ...
                k = pIdx.aiColumn[
                    nEq]; /* Column for inequality constraints */
                ...
                */
                ...
                {
                    pRangeEnd = findTerm(pWC, iCur, k, notReady,
                        WO_LT | WO_LE, pIdx);
                    nExtraReg = 1;
                }
                ...
                {
                    pRangeStart = findTerm(pWC, iCur, k, notReady,
                        WO_GT | WO_GE, pIdx);
                    nExtraReg = 1;
                }
                ...
                */
                if (bRev == (pIdx.aSortOrder[nEq] == SQLITE_SO_ASC
                    ? 1
                    : 0)) SWAP(ref pRangeEnd, ref pRangeStart);

                testcase(pRangeStart != null &&
                         (pRangeStart.eOperator & WO_LE) != 0);
                testcase(pRangeStart != null &&
                         (pRangeStart.eOperator & WO_GE) != 0);
                testcase(pRangeEnd != null &&
                         (pRangeEnd.eOperator & WO_LE) != 0);
                testcase(pRangeEnd != null &&
                         (pRangeEnd.eOperator & WO_GE) != 0);
                ...
                endEq = null == pRangeEnd ||
                        (pRangeEnd.eOperator & (WO_LE | WO_GE)) != 0
                    ? 1
                    : 0;
                start_constraints = pRangeStart != null || nEq > 0
                    ? 1
                    : 0;
                ...
                if (pRangeStart != null)
                {
                    var pRight = pRangeStart.pExpr.pRight;
                ...
                codeApplyAffinity(pParse, regBase,
                    nConstraint, zAff.ToString());
                op = aStartOp[
                    (start_constraints << 2) +
                    (startEq << 1) + bRev];
                Debug.Assert(op != 0);
                ...
                sqlite3VdbeAddOp4Int(v, op, iIdxCur,
                    addrNxt, regBase, nConstraint);
                ...
                */
                ...
                if (pRangeEnd != null)
                {
                    var pRight = pRangeEnd.pExpr.pRight;
                    sqlite3ExprCacheRemove(pParse,
                        regBase + nEq, 1);
                                                                                             ...
                                                                                             WHERE_MULTI_OR) != 0)
            {
                ...
                */
                WhereClause
                    pOrWc; /* The OR-clause broken out into subterms */
                WhereTerm
                    pFinal; /* Final subterm within the OR-clause. */
                SrcList
                    pOrTab; /* Shortened table list or OR-clause generation */

                var regReturn =
                    ++pParse
                        .nMem; /* Register used with OP_Gosub */
                var regRowset =
                ...
                var iLoopBody =
                    sqlite3VdbeMakeLabel(
                        v); /* Start of loop body */
                int
                    iRetInit; /* Address of regReturn init */
                var untestedTerms =
                ...
                */
                ...
                {
                    int
                        nNotReady; /* The number of notReady tables */
                    SrcList_item[]
                        origSrc; /* Original list of tables */
                    nNotReady =
                        pWInfo.nLevel -
                        iLevel - 1;
                    ...
                    pOrTab.nSrc =
                        pOrTab.nAlloc;
                    pOrTab.a[0] =
                        pTabItem; //memcpy(pOrTab.a, pTabItem, sizeof(*pTabItem));
                    origSrc = pWInfo
                        .pTabList.a;
                    for (k = 1;
                        k <= nNotReady;
                            ...
                            origSrc[
                                pWInfo
                                    .a[
                                        iLevel +
                                        k]
                                    .iFrom]; // memcpy(&pOrTab.a[k], &origSrc[pLevel[k].iFrom], sizeof(pOrTab.a[k]));
                }
                else
                {
                    pOrTab = pWInfo
                    ...
                    ii++)
                {
                    ...
                    {
                        WhereInfo
                            pSubWInfo; /* Info for single OR-term scan */
                ...
                if (pWInfo.nLevel > 1)
                    sqlite3DbFree(
                        pParse.db,
                        ref
                        pOrTab); //sqlite3DbFree(pParse.db, pOrTab)
                if (0 == untestedTerms)
                    disableTerm(pLevel,
                        pTerm);
            }
...
*/
    ...
    */
        ...
        */
        private static WhereInfo
            sqlite3WhereBegin(
                Parse
                    pParse, /* The parser context */
                SrcList
                    pTabList, /* A list of all tables to be scanned */
                Expr
                    pWhere, /* The WHERE clause */
                ref ExprList
                    ppOrderBy, /* An ORDER BY clause, or NULL */
                ushort
                    wctrlFlags /* One of the WHERE_* flags defined in sqliteInt.h */
            )
        {
            int
                i; /* Loop counter */
            int
                nByteWInfo; /* Num. bytes allocated for WhereInfo struct */
            int
                nTabList; /* Number of elements in pTabList */
            WhereInfo
                pWInfo; /* Will become the return value of this function */
            var v = pParse
                .pVdbe; /* The virtual data_base engine */
            ulong
                notReady; /* Cursors that are not yet positioned */
            WhereMaskSet
                pMaskSet; /* The expression mask set */
            var pWC =
                new
                    WhereClause(); /* Decomposition of the WHERE clause */
            SrcList_item
                pTabItem; /* A single entry from pTabList */
            WhereLevel
                pLevel; /* A single level in the pWInfo list */
            int
                iFrom; /* First unused FROM clause element */
            int
                andFlags; /* AND-ed combination of all pWC.a[].wtFlags */
            sqlite3
            ...
            */
            ...
            pWInfo.pWC =
                pWC =
                    new
                        WhereClause(); // (WhereClause )((u8 )pWInfo)[nByteWInfo];
            pWInfo.wctrlFlags =
                wctrlFlags;
            ...
            */
            pMaskSet =
                new
                    WhereMaskSet(); //initMaskSet(pMaskSet);
            whereClauseInit(pWC,
                pParse, pMaskSet);
            sqlite3ExprCodeConstants(
                pParse, pWhere);
            whereSplit(pWC, pWhere,
                TK_AND);
      ...
      WHERETRACE("*** Optimizer Start ***\n");
                ...
                i++) //, pLevel++ )
            {
                ...
                pLevel = pWInfo
                    .a[i];
                WhereCost
                    bestPlan; /* Most efficient plan seen so far */
                Index
                    pIdx; /* Index for FROM table at pTabItem */
                int
                    j; /* For looping over FROM tables */
                var bestJ =
                    -1; /* The value of j */
                ulong
                    m; /* Bitmask value for j or bestJ */
                int
                    isOptimal; /* Iterator for optimal/non-optimal search */

                bestPlan =
                    new
                        WhereCost(); // memset( &bestPlan, 0, sizeof( bestPlan ) );
                bestPlan.rCost =
                    SQLITE_BIG_DBL;
                        ...
                        pTabItem =
                            pTabList
                                .a[
                                    j];
                        int
                            doNotReorder; /* True if this table should not be reordered */
                        var sCost =
                            new
                                WhereCost(); /* Cost information from best[Virtual]Index() */
                        ExprList
                            pOrderBy; /* ORDER BY clause for index to optimize */
            ...
            */
                ...
                i++) //, pLevel++ )
            {
                ...
                Table
                    pTab; /* Table to open */
                int
                    iDb; /* Index of data_base containing table/index */
        ...
        */
                ...
                WhereInfo pWInfo)
        {
            ...
            */
            ...
            for (i = 0;
                i < pWInfo.nLevel;
                i++) //  for(i=0, pLevel=pWInfo->a; i<pWInfo->nLevel; i++, pLevel++){
            {
                pLevel = pWInfo
                    .a[i];
                var pTabItem =
